		...
		<view class="uv-calendar__box">
			...
			<view class="uv-calendar__weeks uv-calendar__weeks-week">
				<view
					...
					:key="index"
					class="uv-calendar__weeks-day"
				>
					<text class="uv-calendar__weeks-day-text">{{label.text}}</text>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			// 根据 firstDayOfWeek 生成周标题数组
			weekDayLabels() {
				const labels = [
					{ day: 0, text: this.SUNText },
					{ day: 1, text: this.monText },
					{ day: 2, text: this.TUEText },
					{ day: 3, text: this.WEDText },
					{ day: 4, text: this.THUText },
					{ day: 5, text: this.FRIText },
					{ day: 6, text: this.SATText }
				]
				// 根据 firstDayOfWeek 重新排序
				const sortedLabels = []
				for (let i = 0; i < 7; i++) {
