...
<script>
	...
	export default {
		...
		emits: ['close', 'confirm', 'change', 'monthSwitch', 'choiceDate'],
		props: {
			// 取消按钮颜色
			cancelColor: {
				type: String,
			...
			// 是否自动选择整周（range模式下，点击日期后自动选择该日期所在周）
			autoSelectWeek: {
				type: Boolean,
				default: false
			},
		...
		watch: {
			...
			firstDayOfWeek() {
				// 当周起始日改变时，重新初始化日历
				if (this.cale) {
					this.cale.firstDayOfWeek = this.firstDayOfWeek
					this.cale.setDate(this.nowDate.fullDate)
					this.weeks = this.cale.weeks
				}
			}
		...
		methods: {
			...
			calcWeekRange(dateStr) {
				...
				// 计算该日期所在周的周五（使用与 time.getWeekdayOfWeek 相同的逻辑）
				// targetWeekday = 5 (周五)
				...
				lastSaturday.setDate(friday.getDate() - 6)
			...
			setDateRange(startDate, endDate) {
				...
				// 设置范围选择状态
				this.cale.rangeStatus.before = startDate
				this.cale.rangeStatus.after = endDate
				if (this.cale.dateCompare(startDate, endDate)) {
					this.cale.rangeStatus.data = this.cale.geDateAll(startDate, endDate)
				} else {
					this.cale.rangeStatus.data = this.cale.geDateAll(endDate, startDate)
				}
				// 重新渲染日历
				this.cale.setDate(startDate)
				this.weeks = this.cale.weeks
				this.nowDate = this.cale.getInfo(startDate)
				this.changeConfirmStatus()
			}
		}
	}
