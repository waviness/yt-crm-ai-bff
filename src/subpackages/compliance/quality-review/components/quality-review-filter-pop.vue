...
import type { QualityReviewFilterData } from '../types';

interface Props {
  formData: QualityReviewFilterData;
  isJustSelf: boolean;
...
interface Emits {
  (e: 'confirm', data: QualityReviewFilterData): void;
  (e: 'reset'): void;
}
...
// 筛选值
const filterVal = ref<QualityReviewFilterData>({
  ...props.formData,
});
...
const customSearchValue = ref(''); // 客户搜索关键字
const customList = ref<any[]>([]); // 客户列表
      ...
      <up-form>
        ...
        <app-form-item label="药品信息" :borderBottom="false">
          <up-input
            ...
            clearable
            placeholder="输入药品ID|名称|商品名|客户名称可查询"
            inputAlign="right"
            fontSize="28rpx"
            border="none"
          >
